...
class SsoAuthenticationController extends AbstractActionController
{
    ...
    public function redirectAction()
    {
        ...
        error_log("==================== [SSO REDIRECT DEBUG] ====================");
        error_log("Redirect URI transmise : " . $redirectUri);
        error_log("State généré           : " . $state);
        error_log("URL complète générée   : " . $url);
        error_log("=============================================================");
    ...
    public function callbackAction()
    {
        ...
        error_log("==================== [SSO CALLBACK ARRIVÉE] ====================");
        error_log("Query Params reçus : " . json_encode($queryParams, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
        ...
        if (!empty($queryParams['error'])) {
            ...
            error_log("[SSO ERROR FROM PROVIDER] Erreur: {$err} | Description: {$errDesc}");
            error_log("===============================================================");
        ...
        if (empty($code)) {
            error_log("[SSO ERROR] Le paramètre 'code' est absent du retour SSO.");
            error_log("===============================================================");
        ...
        try {
            ...
            error_log("[SSO SUCCESS] Données reçues : " . json_encode($externalDto->attributes, JSON_PRETTY_PRINT));
            ...
            error_log("[SSO REDIRECT FRONTEND] URL : " . $targetUrl);
            error_log("===============================================================");
