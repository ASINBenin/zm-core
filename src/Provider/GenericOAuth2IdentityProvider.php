...
class GenericOAuth2IdentityProvider implements IdentityProviderInterface
{
    ...
    {
        error_log("[OAUTH2 STEP 1] Appel Token Endpoint : " . $this->config['token_url']);
        error_log("[OAUTH2 STEP 1] Client ID : " . $this->config['client_id'] . " | Redirect URI : " . $redirectUri);
        ...
        error_log("[OAUTH2 STEP 1] Réponse Token Endpoint HTTP {$httpCode} : " . $response);
        if ($curlError) {
            error_log("[OAUTH2 STEP 1 CURL ERROR] " . $curlError);
        }
        ...
        if (!empty($tokenData['id_token'])) {
            ...
            if (count($parts) >= 2) {
                ...
                if (is_array($jwtPayload)) {
                    error_log("[OAUTH2 STEP 2] Payload id_token : " . json_encode($jwtPayload));
        ...
        if (!empty($this->config['userinfo_url'])) {
            error_log("[OAUTH2 STEP 3] Appel UserInfo Endpoint : " . $this->config['userinfo_url']);
            ...
            error_log("[OAUTH2 STEP 3] Réponse UserInfo HTTP {$uHttpCode} : " . $userinfoResponse);
